...
pos <- c(min(grep("Country", unlist(x[6, ]))), min(grep("Gender", unlist(x[3, ]))), min(grep("Maternal", 
    unlist(x[3, ]))), min(grep("seats", unlist(x[3, ]))), min(grep("Labour", unlist(x[3, ]))) - 2, min(grep("Labour", 
    unlist(x[3, ]))))
x <- x[, pos]

names(x) <- c("iso3c", trim(unlist(x[3, ])[-1]))
