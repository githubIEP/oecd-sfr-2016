source("./lib/funcs.R")
indicators <- raw.data$log %>% filter(source == "UNDP/HDI")
filename <- file.path("data", "sfr model data", "2015_statistical_annex_tables_all.xls")
x <- read_excel(filename, "Table 13")
pos <- c(min(grep("Country", unlist(x[5, ]))), min(grep("Vulnerable", unlist(x[4, ]))))
x <- x[, pos]
x$year <- 2013
